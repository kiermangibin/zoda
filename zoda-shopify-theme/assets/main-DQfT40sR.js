...
const globals = {
  ...
  quick_add_initial_load: app.quick_add_initial_load,
  // {boolean} To check if the quick add is loaded for the first time
  // Alert
...
const products = {
  ...
  fetchAndUpdateOptions(sectionId, productHandle, optionPosition, optionValueId, initialVariant, enableDefault = true, sellingPlanGroupId = 0, sellingPlanId = 0) {
    ...
    fetch(
      ...
      if (optionsElement == null ? void 0 : optionsElement.parentNode) {
  ...
  galleryScrollToIndex(index) {
    this.galleryResetZoom();
    let formContainer = this.$refs.formContainer;
    let slider = this.getVisibleGalleryElement(formContainer, ".js-slider");
    let thumbnailSlider = this.getVisibleGalleryElement(
      formContainer,
      ".js-thumbnailSlider"
    );
  ...
  // Get the visible gallery element (desktop/mobile)
  getVisibleGalleryElement(container, selector) {
    const matches2 = Array.from(container.querySelectorAll(selector));
    return matches2.find(
...
const SCROLL_UP_HIDE_PX = 220;
const SCROLL_UP_SHOW_PX = 180;
const utils = {
  ...
  handleBodyScroll() {
    ...
    setTimeout(() => {
      ...
      if (this.is_scrolled_up) {
        if (y > SCROLL_UP_HIDE_PX) {
